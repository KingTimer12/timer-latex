...
#[derive(Default)]
pub struct TexlabState {
    child: Option<CommandChild>,
}

pub type SharedTexlabState = Arc<Mutex<TexlabState>>;

#[tauri::command]
pub async fn start_texlab(app: AppHandle) -> Result<(), String> {
            ...
            match event {
                CommandEvent::Stdout(bytes) => {
                    ...
                    loop {
                        ...
                        let sep = b"\r\n\r\n";
                        let Some(header_end) = raw_buf.windows(sep.len()).position(|w| w == sep)
                        else {
                            break;
                        };
...
#[tauri::command]
pub async fn send_to_texlab(app: AppHandle, message: String) -> Result<(), String> {
    let state = app.state::<SharedTexlabState>();
    let mut guard = state.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn stop_texlab(app: AppHandle) -> Result<(), String> {
    let state = app.state::<SharedTexlabState>();
    let mut guard = state.lock().map_err(|e| e.to_string())?;
